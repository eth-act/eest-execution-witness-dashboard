...
export FILLER_PATH FORK FIXTURES_DIR HIVE_RESULTS_DIR HIVE_SIMULATOR HIVE_PARALLELISM SITE_DIR
...
eest_dashboard_print_env() {
  ...
  for name in \
    ...
    FILLER_PATH FORK FIXTURES_DIR HIVE_RESULTS_DIR HIVE_SIMULATOR HIVE_PARALLELISM SITE_DIR
  do
    eval "value=\${$name}"
    printf '%s=%s\n' "$name" "$value"
  done
