...
export FILLER_PATH FORK FIXTURES_DIR HIVE_RESULTS_DIR HIVE_CLIENT_RESULTS_DIR HIVE_SIMULATOR HIVE_PARALLELISM HIVE_CONSUME_ALLOW_FAILURE SITE_DIR SITE_MAX_SIZE_MB
...
eest_dashboard_print_env() {
  ...
  for name in \
    ...
    FILLER_PATH FORK FIXTURES_DIR HIVE_RESULTS_DIR HIVE_CLIENT_RESULTS_DIR HIVE_SIMULATOR HIVE_PARALLELISM HIVE_CONSUME_ALLOW_FAILURE SITE_DIR SITE_MAX_SIZE_MB
  do
    eval "value=\${$name}"
    printf '%s=%s\n' "$name" "$value"
  done
